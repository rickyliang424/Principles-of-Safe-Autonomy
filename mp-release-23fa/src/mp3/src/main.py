...
def main(argv, window_width, window_height, num_particles, sensor_limit, measurement_noise):
    ...
    pf = particleFilter(bob = bob, world = world, num_particles = num_particles, sensor_limit = sensor_limit,
                        x_start = x_start, y_start = y_start, argv=argv)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_prefix', type = str, help = 'prefix of vis output', default = False)
    parser.add_argument('--quadrant', type = int, default = -1)
    
    argv = parser.parse_args()
    assert argv.quadrant <= 4
    ...
    main(argv, window_width = window_width, window_height = window_height, num_particles = num_particles, sensor_limit = sensor_limit, measurement_noise = measurement_noise)